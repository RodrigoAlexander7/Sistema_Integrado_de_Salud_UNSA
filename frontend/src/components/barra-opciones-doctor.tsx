...
const BarraOpciones: React.FC = () => {
    ...
    const handlePacientesEnEspera = (e: React.FormEvent) => {
            ...
            navigate("/pacientesEsperaDoc");
    }
    const handleInicioDoc = (e: React.FormEvent) => {
            e.preventDefault();
            navigate("/inicio");
    } 
    return (
        ...
        <nav className="flex items-center gap-5 w-full md:w-auto justify-end">
            ...
            <Button variant="ghost" size="icon" className="scale-200 hover:scale-220 transition-transform duration-200" onClick={handleInicioDoc}>
